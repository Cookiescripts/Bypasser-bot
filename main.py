...
async def bypass_link(url: str) -> str:
    async with aiohttp.ClientSession() as session:
        for api in BYPASS_APIS:
            try:
                async with session.get(api.format(url), timeout=aiohttp.ClientTimeout(total=15)) as resp:
                    if resp.status == 200:
...
async def bulkbypass(interaction: discord.Interaction, links: str):
    ...
    embed = discord.Embed(
        title=f"Bulk Bypass Results ({len(unique)} links)",
    ...
    for i, (original, bypassed) in enumerate(zip(unique, results), start=1):
        if bypassed:
            ...
            embed.add_field(
                name=f"Link {i} — Failed",
                value=f"**Original:** {original}\n**Result:** Could not bypass this link.",
                inline=False
            )
...
async def history(interaction: discord.Interaction):
    ...
    if not entries:
        ...
        return
    embed = discord.Embed(
        title=f"Your Bypass History (last {len(entries)})",
